...
export async function GET(req: NextRequest, { params }: { params: { taskId: string } }) {
  ...
  const { data: attempt, error: attemptError } = await admin.from("vto_attempts").select("*, participants!vto_attempts_participant_id_fkey!inner(id,event_id,role)").eq("id", lookId).eq("task_id", params.taskId).eq("participants.role", "bride").maybeSingle();  if (attemptError) console.error("bridal-look/status lookup failed", attemptError);
  ...
  try {
    const result = await getClothTaskStatus(params.taskId);
    ...
    const uploaded = await uploadToStorage(result.resultImageUrl, { folder: "vto-outputs/bride", publicId: `${attempt.participant_id}-${params.taskId}` });
