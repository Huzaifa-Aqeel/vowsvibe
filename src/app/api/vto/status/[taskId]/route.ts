...
export async function GET(req: NextRequest, { params }: { params: { taskId: string } }) {
  ...
  const { data: attempt } = await admin.from("vto_attempts").select("*").eq("participant_id", participantId).eq("task_id", params.taskId).maybeSingle();
  ...
  try {
    ...
    const uploaded = await uploadToStorage(result.resultImageUrl, { folder, publicId: `${participantId}-${params.taskId}` });
