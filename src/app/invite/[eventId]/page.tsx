...
export default async function InvitePage({ params }: { params: Promise<{ eventId: string }> }) {
  ...
  const { data: event } = await admin.from("events").select("*").eq("invite_code", eventId).maybeSingle<EventRow>();
  if (!event) notFound();
  ...
  return <main className="lineup-page-safe-invite relative min-h-screen overflow-hidden bg-[#FBF9F5]"><div aria-hidden="true" className="pointer-events-none absolute inset-0 -z-10"><div className="absolute inset-0 bg-[#FBF9F5]" /><div className="absolute -top-40 left-1/2 h-[36rem] w-[36rem] -translate-x-1/2 rounded-full bg-blush-100/50 blur-[110px]" /><div className="absolute bottom-0 right-0 h-[28rem] w-[28rem] translate-x-1/3 translate-y-1/3 rounded-full bg-blush-200/30 blur-[100px]" /></div><BridesmaidFlow event={{ ...event, example_dresses }} /></main>;
}
