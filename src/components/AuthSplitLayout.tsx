...
export function AuthSplitLayout({ children, eyebrow = "A calmer way to coordinate the wedding party" }: { children: ReactNode; eyebrow?: string }) {
  return (
      ...
      <div className="grid min-h-screen lg:grid-cols-[1.05fr_0.95fr]">
        <section className="relative hidden overflow-hidden bg-stone-950 px-10 py-12 text-white lg:flex lg:flex-col lg:justify-between xl:px-16">
          ...
          <div className="relative">
            <div className="flex items-center gap-2 text-sm font-semibold tracking-wide"><span className="flex h-8 w-8 items-center justify-center rounded-full bg-white/10"><Sparkles size={15} /></span> Vows &amp; Vibe</div>
            <div className="mt-24 max-w-xl">
              <p className="mb-4 text-[10px] font-bold uppercase tracking-[0.3em] text-rose-200">{eyebrow}</p>
              <h1 className="font-serif text-5xl leading-[1.04] xl:text-6xl">One beautiful place for the <em className="font-normal text-rose-200">whole look.</em></h1>
              <p className="mt-6 max-w-lg text-sm leading-7 text-stone-300">Set the dress direction, let each bridesmaid create a private AI try-on, then build the final bridal-party lineup from the looks everyone actually confirms.</p>
            </div>
          </div>
          <div className="relative grid max-w-xl grid-cols-3 gap-3">
            {[{ icon: Shirt, n: "01", title: "Set the style", text: "Palette, dress and mood." }, { icon: Camera, n: "02", title: "Try it on", text: "Private AI fitting." }, { icon: Users, n: "03", title: "Build the lineup", text: "Confirmed looks only." }].map(({ icon: Icon, n, title, text }) => (
